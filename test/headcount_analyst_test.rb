...
class HeadcountAnalystTest < Minitest::Test
  ...
  def test_finds_a_single_statewide_leader_in_a_single_subject
    expected = 0.123
    assert_equal 0.123, @ha.top_statewide_testing_year_over_year_growth(:subject => :math)
  end
  ...
  def test_returns_how_a_districts_kindergarten_participation_rate_compares_to_the_state_average
    expected = 0.123
    assert_equal expected, @ha.kindergarten_participation_rate_variation('district_name', :against => 'state')
  end

  def test_returns_how_does_a_districts_kindergarten_participation_rate_compare_to_another_district
    expected = 0.123
    assert_equal expected @ha.kindergarten_participation_rate_variation('district_name', :against => 'second_district')
  end

  def test_returns_how_kindergarten_participation_variation_compares_to_the_median_household_income_variation
    expected = 1.2
    assert_equal expected, @ha.kindergarten_participation_against_household_income('district_name')
  end

  def test_returns_how_kindergarten_participation_correlates_with_median_household_income
    assert @ha.kindergarten_participation_correlates_with_household_income(:for => 'district name')
  ...
  def test_returns_how_kindergarten_participation_variation_compares_to_high_school_graduation_variation
    expected = 1.2
    assert_equal expected, @ha.kindergarten_participation_against_high_school_graduation('district_name')
  end
